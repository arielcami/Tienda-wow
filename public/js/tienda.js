    ...
    try {
        ...
        if (response.data.success && response.data.categories.length > 0) {
            ...
            if (categoriesList.length > 0) {
                await loadItemsByCategory(categoriesList[0]);
            }
        } else {
...
function displayItemsByCategory(items) {
    ...
    container.innerHTML = items.map(item => {
        ...
        return `
            <div class="item-card rounded-xl p-4 border-gold cursor-pointer" onclick="showSimpleItemDetails(${item.entry})">
...
async function showSimpleItemDetails(entry) {
    try {
        ...
        // Configurar imagen del tooltip
        const tooltipImage = document.getElementById('simple-modal-tooltip-image');
        if (item.tooltip_image) {
            tooltipImage.src = `/tooltips/${item.tooltip_image}`;
            tooltipImage.alt = `Tooltip de ${item.name}`;
        } else {
            tooltipImage.src = '/icons/inv_misc_questionmark.jpg';
            tooltipImage.alt = 'Tooltip no disponible';
        }
        ...
        Swal.fire({
            ...
            html: `
                    <div class="text-left">
                        ...
                        <p><strong>Personaje:</strong> ${character.name} (Nivel ${character.level})</p>
        ...
        await Swal.fire({
            ...
            html: `
                    <div class="text-left">
                        ...
                        <p><strong>Personaje:</strong> ${character.name}</p>
